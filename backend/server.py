...
def _sync_append_intake(intake: 'IntakeCreate') -> None:
    ...
    try:
        ...
        row = [
            ts,                                           # 1  timestamp
            intake.full_name,                             # 2  full_name
            intake.birthday or '',                        # 3  birthday
            intake.age or '',                             # 4  age
            intake.phone or '',                           # 5  phone
            str(intake.email) if intake.email else '',    # 6  email
            '',                                           # 7  blank
            'Yes' if intake.heart_disease else 'No',      # 8  heart_disease
            'Yes' if intake.high_blood_pressure else 'No',# 9  high_blood_pressure
            'Yes' if intake.varicose_veins else 'No',     # 10 varicose_veins
            'Yes' if intake.pregnant else 'No',           # 11 pregnant
            intake.recent_injuries or '',                 # 12 recent_injuries
            intake.other_complaints or '',                # 13 other_complaints
            '',                                           # 14 blank
            '',                                           # 15 blank
            intake.client_name or '',                     # 16 client_name
